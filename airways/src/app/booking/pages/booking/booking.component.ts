...
export class BookingComponent implements OnInit {
  ...
  getFlightData() {
    ...
    this.airportService
      .searchFlight(this.generateFlightRequest())
      .subscribe((flight) => {
        const products = flight;
        const flightFrom = this.getArrayOfFlight(
          products[0].otherFlights,
          products[0]
        );
        if (this.returnTicket) {
          const flightTo = this.getArrayOfFlight(
            products[1].otherFlights,
            products[1]
          );
  ...
  getArrayOfFlight(
    ...
    for (let index = 0; index < numberTicket.length; index += 1) {
      ...
      if (!keys.includes(element)) {
        arrayOfFlight.push([
          element,
          { takeoffDate: this.createDate(element, ticket.takeoffDate) },
        ]);
      }
    }
